...
contract("Gartic", accounts => {
    ...
    context("Put a word tests", function() {
        ...
        it("should always reverts with the right reason", async function() {
            await truffleAssertions.reverts(this.gartic.putNewWord(words[2], {from: player2}), "address already played");
            await truffleAssertions.reverts(this.gartic.putNewWord(words[1], {from: accounts[2]}), "word is the same than the previous word");
            await truffleAssertions.reverts(this.gartic.putNewWord("", {from: accounts[2]}), "word can't be empty");
        })
    })
    ...
    context("retrieving words tests", function() {
        ...
        it("should revert transaction with: Ownable: caller is not the owner", async function() {
           await truffleAssertions.reverts(this.gartic.getAllWords.call({from: player2}), 'Ownable: caller is not the owner');
        })
    ...
    context("late game tests", function() {
        ...
        it("should put the last word and change the game state", async function() {
            ...
            truffleAssertions.eventEmitted(result, 'onStateChange', (ev) => {
        ...
        it("should try to put a word and revert", async function() {
            await truffleAssertions.reverts(this.gartic.putNewWord("tooMuch", {from: accounts[20]}), "you can't enter word anymore");
        })
        it("should try a word and don't emit the end of game", async function() {
            ...
            //guessCount: 0 -> 1
            truffleAssertions.eventNotEmitted(result, 'onFinishedGame');
        })
        it("should try a word and emit the end of game and state change", async function() {
            ...
            truffleAssertions.eventEmitted(result, 'onFinishedGame', (ev) => {
